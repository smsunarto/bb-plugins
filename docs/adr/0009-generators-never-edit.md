...
`server/server.ts`; `bb-kit check` fails until the wiring exists.

`check`'s scope is composition wiring (the router matches the unit files
beside `bb.server`, procedures are statically discoverable, no duplicate
wire methods, namespace equals the plugin id) plus manifest sanity (entry
targets exist and do not point at build output, engines pins are well-formed). Workspace policy —
...
Rejected: guarded AST edits (ts-morph editing `server/server.ts`, refusing
...
- `server/server.ts` is written only by its author; there is no tool-ownership
