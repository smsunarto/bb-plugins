...
// `dist/bin/bin.js` leaves `node_modules/.bin/bb-kit` unlinked on a clean
// checkout, because Bun links bins during install only. This file is
// committed, so the link always exists; it resolves `dist/bin/bin.js` at run
// time, by which point `build` has emitted it.
import "../dist/bin/bin.js";
