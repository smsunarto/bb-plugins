...
function isTypeOnlyImport(
  declaration: ReturnType<
    ReturnType<Project["addSourceFileAtPath"]>["getImportDeclarations"]
  >[number],
): boolean {
...
function checkFrameworkDependencies(info: ProjectInfo): Diagnostic[] {
  ...
  if (info.modules.some((module) => module.operations.length > 0) && !dependencies.zod) {
    diagnostics.push(
      diagnostic(
        "BBK009",
...
function checkMigrations(info: ProjectInfo): Diagnostic[] {
  ...
  for (const module of info.modules) {
    ...
    if (catalogComparable) {
      ...
      const expected = migrationCatalogSource(module.name, module.directory, module.migrations);
      if (!existsSync(catalogPath) || readFileSync(catalogPath, "utf8") !== expected) {
        diagnostics.push(
          diagnostic(
            "BBK305",
...
function checkImports(info: ProjectInfo): Diagnostic[] {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    for (const declaration of source.getImportDeclarations()) {
      ...
      if (packageName && shimmedPackageRoots.has(packageName) && !exactHostShims.has(specifier)) {
        diagnostics.push(
          diagnostic(
            "BBK112",
