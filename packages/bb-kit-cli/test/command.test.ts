...
describe("bb-kit command interface", () => {
  ...
  it("requires confirmation before destructive invocation", async () => {
    ...
    makeOperationRequireInput(root, "reports.delete", "command", { id: "R-1" }, "destructive");
    const request = vi.fn<typeof fetch>();
    const output = capture();
    expect(
      await runCli(["invoke", "reports.delete", "--input", '{"id":"R-1"}', "--json"], {
        cwd: root,
        io: output.io,
        fetch: request,
      }),
  ...
  it("invokes the locked native RPC method without a GUI", async () => {
    ...
    makeOperationRequireInput(root, "reports.refresh", "command", { scope: "all" }, "mutating");
    const request = vi.fn<typeof fetch>(
      async () =>
        new Response(
          JSON.stringify({
  ...
  it("sends null only for omitted canonical no-input operations", async () => {
    ...
    const output = capture();
    expect(
      await runCli(["invoke", "reports.get", "--json"], {
        cwd: root,
        io: output.io,
        fetch: request,
      }),
    ...
    const extra = capture();
    expect(
      await runCli(["invoke", "reports.get", "--input", "null", "--json"], {
        cwd: root,
        io: extra.io,
        fetch: request,
      }),
  ...
  it("requires explicit input for every other schema", async () => {
    ...
    expect(
      await runCli(["invoke", "reports.nullable", "--json"], {
        cwd: root,
        io: missing.io,
        fetch: request,
      }),
    ...
    expect(JSON.parse(missing.stdout[0] ?? "null")).toEqual({
      ...
      error: {
        ...
        message:
          "reports.nullable requires input. Example: null. Run: bb-kit invoke reports.nullable --input 'null'",
      },
    });
    expect(request).not.toHaveBeenCalled();
    ...
    expect(
      await runCli(["invoke", "reports.nullable", "--input", "null", "--json"], {
        cwd: root,
        io: explicit.io,
        fetch: request,
      }),
  ...
  it("keeps init option values out of the target path", async () => {
    ...
    const output = capture();
    expect(
      await runCli(["init", "--kind", "fullstack", "--skip-types", "--skip-install", "--json"], {
        cwd: parent,
        io: output.io,
      }),
    ).toBe(0);
  ...
  it("exposes the complete verification gate as stable JSON", async () => {
    ...
    const run = vi.fn<CommandRunner>((request) => {
      ...
      return commandResult({
        stdout:
          request.args[0] === "pm"
            ? [...paths.map((path) => `packed 1KB ${path}`), `Total files: ${paths.length}`].join(
                "\n",
              )
            : "",
      });
