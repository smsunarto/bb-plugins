...
- Amp stream messages are translated to ACP `session/update` notifications: text → `agent_message_chunk`, thinking → `agent_thought_chunk` (the bridge always runs with `thinking: true`), images → ACP image content, `tool_use` → `tool_call`, `tool_result` → `tool_call_update`. When Oracle starts, its input is persisted under `$XDG_STATE_HOME/bb-plugin-amp/oracle` and an `amp-oracle` card appears immediately; messages linked by `parent_tool_use_id` update its live thinking/tool trace until the final result replaces the running view with Oracle's answer. The native tool events remain as a fallback. Amp emits whole messages per line — there is no token-level streaming to forward.
