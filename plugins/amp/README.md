...
- Amp stream messages are translated to ACP `session/update` notifications: text → `agent_message_chunk`, thinking → `agent_thought_chunk` (the bridge always runs with `thinking: true`), images → ACP image content, `tool_use` → `tool_call`, `tool_result` → `tool_call_update`. When Oracle starts, its input and messages linked by `parent_tool_use_id` are persisted under `$XDG_STATE_HOME/bb-plugin-amp/oracle`. The completed `amp-oracle` card is appended as the turn's terminal assistant content, which keeps it visible in the chat log instead of inside bb's collapsed "Working" disclosure. The native tool events remain as a fallback. Amp emits whole messages per line — there is no token-level streaming to forward.
