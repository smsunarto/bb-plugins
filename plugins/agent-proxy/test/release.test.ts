...
test("repository settings normalize supported GitHub source forms", () => {
  ...
  assert.equal(
    normalizeCoreRepo("https://github.com/example-owner/CLIProxyAPI.git"),
    "example-owner/CLIProxyAPI",
  );
  assert.equal(
    normalizeCoreRepo("git@github.com:example-owner/CLIProxyAPI.git"),
    "example-owner/CLIProxyAPI",
  );
