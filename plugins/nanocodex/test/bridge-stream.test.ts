...
test("typed AgentEvent objects project onto the canonical bb timeline", () => {
  ...
  projector.consume(event(3, "model.call.completed", {
    call_index: 1,
    usage: { input_tokens: 13, output_tokens: 2, total_tokens: 15 },
  }));
  projector.consume(event(4, "assistant.message", {
    model_call_index: 1,
    phase: "final_answer",
    text: "answer",
  }));
  projector.consume(event(5, "tool.call", {
    call_id: "call-1",
    tool: "bash",
    arguments: { command: "pwd" },
  }));
  projector.consume(event(6, "tool.result", {
    call_id: "call-1",
    tool: "bash",
    status: "completed",
    result: "/workspace",
  ...
  writer.flush();
  assert.equal(deltas(messages).some((delta) => delta.kind === "turn.boundary"), false);

  scribe.settle("completed");
  const projected = deltas(messages);
  assert.deepEqual(raw, []);
  assert.ok(projected.some((delta) => delta.kind === "input.accepted"));
  assert.ok(projected.some((delta) => delta.kind === "item.textDelta" && delta.channel === "reasoningSummary"));
  assert.ok(projected.some((delta) => delta.kind === "item.textDelta" && delta.channel === "agentMessage"));
