...
export async function inspectAuthSeed(
  ...
  try {
    ...
    return { state: "missing", path, message: `Cannot open the Codex auth file: ${errorMessage(error)}` };
  }

  try {
    const stat = await file.stat();
    if (!stat.isFile()) return { state: "broken", path, message: "The Codex auth path is not a file." };
    if (stat.size > MAX_AUTH_FILE_BYTES) {
      return { state: "broken", path, message: `The Codex auth file exceeds ${MAX_AUTH_FILE_BYTES} bytes.` };
    }
    if (process.platform !== "win32" && (stat.mode & 0o077) !== 0) {
      return { state: "broken", path, message: "The Codex auth file is accessible by group or other users." };
    }
    ...
    return { state: "broken", path, message: `Cannot read the Codex auth file: ${errorMessage(error)}` };
  } finally {
    await file.close();
  }
}
...
function parseAuthSeed(encoded: string, path: string, now: number): AuthSeedStatus {
  ...
  try {
    ...
    return { state: "broken", path, message: `Cannot parse the Codex auth file: ${errorMessage(error)}` };
  }
  if (!isRecord(parsed) || parsed.auth_mode !== "chatgpt" || !isRecord(parsed.tokens)) {
    return { state: "missing", path, message: "The Codex auth file has no ChatGPT subscription login." };
  }
  ...
  if (accountId === undefined || accountIds.some((candidate) => candidate !== accountId)) {
    return { state: "broken", path, message: "The Codex auth file has no consistent ChatGPT account ID." };
  }
  ...
  if (expiresAt === undefined || expiresAt <= now + MINIMUM_TTL_MS) {
    return { state: "expired", path, message: "The Codex access token is expired or expires too soon." };
  }
  const fedrampClaims = [
  ...
  if (fedrampClaims.some((candidate) => candidate !== fedrampClaims[0])) {
    return { state: "broken", path, message: "The Codex auth file has conflicting FedRAMP claims." };
  }
