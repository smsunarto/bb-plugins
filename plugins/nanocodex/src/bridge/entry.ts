...
interface BridgeDependencies {
  ...
  readonly createRegistry?: (args: { binding: NativeBinding; storage: NanocodexStorage }) => SessionRegistry;
}
...
export function createBridge(dependencies: BridgeDependencies = {}): NanocodexBridge {
  ...
  const requireRuntime = (): { registry: SessionRegistry; binding: NativeBinding; storage: NanocodexStorage } => {
  ...
  const handlers: Record<string, (id: JsonRpcId, params: unknown) => void | Promise<void>> = {
    ...
    [BRIDGE_REQUEST_METHODS.modelList]: (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.modelList, parsed.error.issues);
    ...
    [BRIDGE_REQUEST_METHODS.providerHealth]: async (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.providerHealth, parsed.error.issues);
      io.sendResult(id, await providerHealth(requireRuntime().binding));
    },
    [BRIDGE_REQUEST_METHODS.providerUsage]: (id) => methodNotFound(id, BRIDGE_REQUEST_METHODS.providerUsage),
    [BRIDGE_REQUEST_METHODS.providerInstallationStatus]: (id) => methodNotFound(id, BRIDGE_REQUEST_METHODS.providerInstallationStatus),
    [BRIDGE_REQUEST_METHODS.providerInstallationRun]: (id) => methodNotFound(id, BRIDGE_REQUEST_METHODS.providerInstallationRun),
    [BRIDGE_REQUEST_METHODS.threadStart]: async (id, params) => {
      const parsed = threadStartParamsSchema.safeParse(params);
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.threadStart, parsed.error.issues);
    ...
    [BRIDGE_REQUEST_METHODS.threadResume]: async (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.threadResume, parsed.error.issues);
    ...
    [BRIDGE_REQUEST_METHODS.threadFork]: async (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.threadFork, parsed.error.issues);
      let seed;
      try {
    ...
    [BRIDGE_REQUEST_METHODS.turnStart]: (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.turnStart, parsed.error.issues);
      let afterReply: () => void;
      try {
    ...
    [BRIDGE_REQUEST_METHODS.turnSteer]: (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.turnSteer, parsed.error.issues);
    ...
    [BRIDGE_REQUEST_METHODS.threadStop]: (id, params) => {
      ...
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.threadStop, parsed.error.issues);
      io.sendResult(id, {});
      void requireRuntime().registry.stop(parsed.data.threadId, parsed.data.intent).catch(() => {});
    },
    [BRIDGE_REQUEST_METHODS.threadDiscard]: (id, params) => {
      const parsed = threadDiscardParamsSchema.safeParse(params);
      if (!parsed.success) return invalidParams(id, BRIDGE_REQUEST_METHODS.threadDiscard, parsed.error.issues);
      io.sendResult(id, {});
      void requireRuntime().registry.discard(parsed.data.threadId, parsed.data.providerThreadId).catch(() => {});
    },
...
async function providerHealth(binding: NativeBinding): Promise<ProviderHealthResult> {
  ...
  switch (status.state) {
    ...
    case "expired":
      return { supported: true, health: { ...base, status: "expired", statusMessage: "The ChatGPT subscription expired. Start device login from provider health." } };
    case "pending":
      return {
        supported: true,
        health: {
    ...
    case "signed_out":
      return { supported: true, health: { ...base, status: "unauthenticated", statusMessage: "NanoCodex is signed out." } };
    case "broken":
      return { supported: true, health: { ...base, status: "unknown", statusMessage: status.message } };
  }
}
