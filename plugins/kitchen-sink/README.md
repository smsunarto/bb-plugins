...
For Unity `.prefab` and `.unity` assets, `::smart-diff` automatically shows an object inspector: GameObjects, hierarchy breadcrumbs, components, and before/after properties. Expand or collapse each object or component, or select **Raw YAML** to inspect the original patch. Line ranges still select the displayed hunks. Object IDs retain their full precision, local references show object names, and external asset references retain their GUIDs. Custom components use `m_EditorClassIdentifier` when present, otherwise `MonoBehaviour` and the file ID.

The object view uses the SDK's old file content and applies the exact patch, then saves the semantic result alongside the immutable diff snapshot. Binary, malformed, truncated, oversized, or unavailable sources retain the YAML view with a notice. Previously saved snapshots and proposed `::smart-patch` embeds retain their original line view. Prefab instance overrides show their serialized target/property paths. This is a serialized property inspector, not a rendered 3D preview.
