...
test("citation keys separate paths, threads and ranges without delimiter collisions", () => {
  const request = { kind: "code", threadId: "t", path: "a.ts" } as const;
  const keys = [
    request,
    { ...request, path: "a.ts 1" },
    { ...request, start: 1 },
    { ...request, end: 1 },
    { ...request, threadId: "t a.ts" },
  ].map(embedCacheKey);
