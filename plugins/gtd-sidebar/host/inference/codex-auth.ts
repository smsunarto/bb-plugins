...
import { z } from "zod";
import { resolveCodexHome } from "./codex-home.ts";
...
export type CodexAuthUnusableReason = "not_object" | "api_key" | "access_token" | "account_id";
...
function codexAuthPath(): string {
  return path.join(resolveCodexHome(os.homedir(), process.env), CODEX_AUTH_FILE_NAME);
}
...
function classifyAuthJson(value: JsonValue): ClassifiedAuthJson {
  ...
  const idTokenClaims = idToken === null ? toJsonObject(tokens.id_token) : chatGptClaims(idToken);
  const accountId =
  ...
  return {
    ...
    credentials: {
      ...
      accessToken,
      accountId,
      accountEmail: accountEmail(accessToken) ?? (idToken === null ? null : accountEmail(idToken)),
...
const UNUSABLE_AUTH_MESSAGES: Record<CodexAuthUnusableReason, (authPath: string) => string> = {
  not_object: (authPath) =>
