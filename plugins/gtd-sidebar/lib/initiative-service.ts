...
>;
type Projects = Pick<BbPluginApi["sdk"]["projects"], "list">;
...
export function createInitiativeService(deps: InitiativeServiceDeps): InitiativeService {
  const { store, threads, projects, pluginId, publish, log, recheckDispatch } = deps;
    ...
    async createInitiative(input) {
      ...
      try {
        ...
        thread = await threads.spawn({
          ...
          title: input.name,
          visibility: "visible",
          environment: environmentSpec(input.environmentId, input.environmentProviderId),
        ...
        initiative = store.create({
          ...
          primaryEnvironmentId: input.environmentId ?? null,
        ...
        // The pending thread has no initiative; delete it so its held first
        // message dies with it instead of surfacing as a ghost row. If the
        // delete itself fails, the marker's fail-closed reject path covers it.
        if (thread !== null) {
