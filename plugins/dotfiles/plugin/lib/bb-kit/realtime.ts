...
export interface RealtimeInvalidationOptions<Schema extends StandardSchemaV1> {
 ...
 */
export function useRealtimeInvalidation<const Schema extends StandardSchemaV1>({
  channel,
  schema,
  keys,
  reconnect = [],
  ...
  useRealtime(
    ...
    useCallback(
      (payload: unknown) => {
        void Promise.resolve(schema["~standard"].validate(payload)).then(async (result) => {
          ...
          await Promise.all(
            keys(result.value).map((queryKey) => queryClient.invalidateQueries({ queryKey })),
          );
          return undefined;
        });
      },
  ...
  useEffect(() => {
    if (previousConnection.current === "reconnecting" && connection === "connected") {
      void Promise.all(reconnect.map((queryKey) => queryClient.invalidateQueries({ queryKey })));
    }
