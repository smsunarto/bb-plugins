...
import { PluginQueryBoundary, useOperationRpc } from "../../lib/bb-kit/query.js";
...
function DotfilesPanelBody() {
  const rpc = useOperationRpc(dotfilesOperations);
  const queryClient = useQueryClient();
  const overview = useQuery(overviewQueryOptions(rpc));
  ...
  const file = useQuery({
    ...fileQueryOptions(rpc, selectedPath ?? ""),
  ...
  async function reloadFile(): Promise<void> {
    ...
    await queryClient.invalidateQueries({
      queryKey: dotfilesKeys.file(selectedPath),
      exact: true,
    });
  }
  ...
  async function save(content: string, expectedSha256: string): Promise<SaveFileResult> {
    ...
    try {
      ...
      queryClient.setQueryData(dotfilesKeys.file(selectedPath), nextFile);
