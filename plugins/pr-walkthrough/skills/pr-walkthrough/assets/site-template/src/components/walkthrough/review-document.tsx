...
export function ReviewDocument({
  ...
  const requiredGuideExcerpts = group.guide.phases
    .flatMap((phase) => phase.excerpts)
    .filter((excerpt) => excerpt.countsTowardCompletion);
  const reviewedGuideCount = requiredGuideExcerpts.filter((excerpt) =>
    reviewedExcerptIds.has(excerpt.id),
  ).length;
  const primaryFiles = useMemo(
    () => groupFiles.filter((file) => !file.generated && !file.binary),
    [groupFiles],
  );
  ...
  const groupProgress =
    reviewMode === "normal"
      ? groupFiles.length > 0
        ? (reviewedCount / groupFiles.length) * 100
        : viewed
          ? 100
          : 0
      : requiredGuideExcerpts.length > 0
        ? (reviewedGuideCount / requiredGuideExcerpts.length) * 100
        : 0;
  const reviewStatus =
    reviewMode === "normal"
      ? groupFiles.length > 0
        ? `${reviewedCount} of ${groupFiles.length} files reviewed`
        : viewed
  ...
  useEffect(() => {
    const root = document.querySelector<HTMLElement>(
      `[data-active-review-group="${CSS.escape(group.id)}"]`,
    );
  ...
  return (
    <main
      className="relative h-full w-full min-h-0 min-w-0 flex-1 overflow-hidden bg-background"
      data-active-review-group={group.id}
          ...
          <header>
            <div className="flex items-start justify-between gap-4">
              ...
              <Toggle
                aria-label={
                  viewed
                    ? "Clear all Normal and Guide review progress"
                    : "Mark all Normal and Guide reviewed"
                }
                ...
                variant="outline"
              >
                {viewed ? <Check className="text-[var(--added)]" /> : null}
                {viewed ? "Reviewed · Normal + Guide" : "Mark Normal + Guide reviewed"}
              </Toggle>
            </div>
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <Progress
                aria-label={`${group.title}: ${reviewStatus}`}
            ...
            >
              <TabsList aria-label="Reading mode" className="rounded-md" variant="default">
                <TabsTrigger className="rounded-sm px-3" value="normal">
                  Normal
                </TabsTrigger>
                <TabsTrigger className="rounded-sm px-3" value="guide">
                  Guide
                </TabsTrigger>
              </TabsList>
            </Tabs>
            ...
            <>
              <div className="mb-2 mt-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-semibold" id="changed-files-heading">
                    Changed files
                  </h2>
                </div>
                <div className="flex items-center gap-2">
                  {generatedFiles.length > 0 ? (
                    <Toggle
                      aria-label={
                        showGeneratedFiles
                          ? "Hide generated and binary files"
                          : "Show generated and binary files"
                      }
                    ...
                    >
                      <FileCog />
                      {showGeneratedFiles ? "Hide generated/binary" : "Show generated/binary"}
                    </Toggle>
                  ) : null}
                  <ToggleGroup
                    ...
                    variant="outline"
                  >
                    <ToggleGroupItem className="rounded-l-md! rounded-r-none!" value="unified">
                      Unified
                    </ToggleGroupItem>
                    <ToggleGroupItem className="rounded-l-none! rounded-r-md!" value="split">
                      Split
                    </ToggleGroupItem>
                  </ToggleGroup>
                </div>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="absolute right-4 bottom-4 z-30 size-11 bg-background/95 shadow-lg backdrop-blur-sm sm:size-8"
              onClick={scrollToTop}
              size="icon"
              variant="outline"
            >
