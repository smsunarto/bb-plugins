...
import {
  walkthroughData,
  type WalkthroughDiffFile,
  type WalkthroughGuideExcerpt,
  type WalkthroughReviewGroup,
...
function groupFiles(group: WalkthroughReviewGroup, diffByPath: Map<string, WalkthroughDiffFile>) {
  return group.files
    .map((file) => diffByPath.get(file.path))
    .filter((file): file is WalkthroughDiffFile => Boolean(file));
}
...
export function WalkthroughApp() {
  ...
  const [selectedDiffPath, setSelectedDiffPath] = useState<string | undefined>(
    walkthroughData.diffFiles[0]?.path,
  );
  ...
  const [expandedPaths, setExpandedPaths] = useState<Set<string>>(
    () =>
      new Set(
        walkthroughData.diffFiles
          .filter((file) => !file.generated && !file.binary)
          .map((file) => file.path),
      ),
  );
  const [expandedExcerptIds, setExpandedExcerptIds] = useState<Set<string>>(
    () =>
      new Set(
        groups
          .flatMap(groupGuideExcerpts)
          .filter((excerpt) => !excerpt.defaultCollapsed)
  ...
  const diffByPath = useMemo(
    () => new Map(walkthroughData.diffFiles.map((file) => [file.path, file])),
    [],
  );
  ...
  const activeGroupFiles = useMemo(
    () => (activeGroup ? groupFiles(activeGroup, diffByPath) : []),
    [activeGroup, diffByPath],
  );
  const activeGuideExcerpts = useMemo(
    () => (activeGroup ? groupGuideExcerpts(activeGroup) : []),
    [activeGroup],
  );
  const allGuideExcerpts = useMemo(() => groups.flatMap(groupGuideExcerpts), [groups]);
  const validDiffPaths = useMemo(
    () => new Set(walkthroughData.diffFiles.map((file) => file.path)),
    [],
  );
  const validExcerptIds = useMemo(
    () => new Set(allGuideExcerpts.map((excerpt) => excerpt.id)),
    [allGuideExcerpts],
  );
  ...
  const resolvedViewedGroups = useMemo(() => {
    ...
    for (const group of groups) {
      ...
      const requiredExcerpts = groupGuideExcerpts(group).filter(
        (excerpt) => excerpt.countsTowardCompletion,
      );
      const normalComplete =
        files.length > 0 && files.every((file) => reviewedPaths.has(file.path));
      const guideComplete =
        requiredExcerpts.length > 0 &&
        requiredExcerpts.every((excerpt) => reviewedExcerptIds.has(excerpt.id));
  ...
  const diffTotals = useMemo(
    () =>
      walkthroughData.diffFiles.reduce(
        (totals, file) => ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored !== null) {
        ...
        if (
          parsed.reviewedPaths !== undefined &&
          (!Array.isArray(parsed.reviewedPaths) ||
            parsed.reviewedPaths.some((path) => typeof path !== "string"))
        ) {
          throw new Error("Saved progress has invalid Normal paths");
        }
        if (
          parsed.reviewedExcerptIds !== undefined &&
          (!Array.isArray(parsed.reviewedExcerptIds) ||
            parsed.reviewedExcerptIds.some((id) => typeof id !== "string"))
        ) {
          ...
          setReviewedPaths(
            new Set(
              parsed.reviewedPaths.filter(
                (path): path is string => typeof path === "string" && validDiffPaths.has(path),
              ),
            ),
          );
        }
          ...
          setReviewedExcerptIds(
            new Set(
              parsed.reviewedExcerptIds.filter(
                (id): id is string => typeof id === "string" && validExcerptIds.has(id),
              ),
            ),
          );
        }
  ...
  const selectGroup = useCallback(
    (groupId: string) => {
      ...
      const initialFile =
        nextGroupFiles.find((file) => !file.generated && !file.binary) ?? nextGroupFiles[0];
      setActiveGroupId(groupId);
      setSelectedDiffPath(initialFile?.path);
      setGroupSheetOpen(false);
      window.requestAnimationFrame(() => {
        const viewport = document.querySelector<HTMLElement>(
          '[data-review-document] [data-slot="scroll-area-viewport"]',
        );
        viewport?.scrollTo({
          top: 0,
  ...
  const toggleViewed = useCallback(() => {
    ...
    setViewedGroups((current) => {
      ...
      else if (activeGroupFiles.length === 0 && activeGuideExcerpts.length === 0)
  ...
  const updateExpandedExcerptIds = useCallback(
    (ids: string[]) => {
  ...
  const openInlineDiff = useCallback((path: string) => {
    ...
    window.setTimeout(() => {
      ...
      document
        .querySelector<HTMLElement>(`[data-review-document] [data-diff-file-path="${escapedPath}"]`)
        ?.scrollIntoView({
          behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches
            ? "auto"
            : "smooth",
  ...
  return (
    <div className="review-shell flex h-svh min-h-0 flex-col overflow-hidden bg-background text-foreground">
      <header className="flex h-12 shrink-0 items-center gap-3 border-b bg-background px-3">
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              aria-label="Open review groups"
              className="-ml-1 size-11 shrink-0 sm:size-8 xl:hidden"
        ...
        <div className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <span className="truncate text-sm text-muted-foreground">
            {walkthroughData.meta.title}
          </span>
          <span
            aria-live="polite"
            className="hidden shrink-0 text-[10px] text-muted-foreground md:inline"
          >
        ...
        <div className="hidden shrink-0 items-center gap-2 text-[11px] text-muted-foreground lg:flex">
          ...
          <span className="font-mono">{walkthroughData.meta.baseRef}</span>
          <ArrowLeft className="size-3" />
          <span className="max-w-44 truncate font-mono">{walkthroughData.meta.headRef}</span>
        ...
        <InputGroup className="hidden h-8 w-[220px] shrink-0 lg:flex">
          ...
          <InputGroupInput
            aria-label="Search review groups"
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Search in PR"
            ref={searchRef}
            value={query}
          />
        ...
        {walkthroughData.meta.prUrl ? (
          <Button asChild className="shrink-0 rounded-md" size="sm">
            <a href={walkthroughData.meta.prUrl} rel="noreferrer" target="_blank">
              Open PR <ExternalLink />
            </a>
          </Button>
        ) : null}
      </header>
      ...
      {persistenceState.kind === "failed" ? (
        <div
          className="flex shrink-0 flex-wrap items-center gap-2 border-b bg-muted/40 px-3 py-2 text-xs text-muted-foreground"
          role="status"
        >
          ...
          <Button onClick={retryPersistence} size="sm" variant="outline">
            Retry
          </Button>
          {persistenceState.stage === "load" ? (
            <Button onClick={resetUnreadableProgress} size="sm" variant="outline">
              Reset saved progress
            </Button>
          ) : null}
          <Button onClick={copyProgressBackup} size="sm" variant="outline">
            Copy progress
          </Button>
          {backupStatus === "copied" ? (
            <span>Copied</span>
          ...
          <ResizablePanelGroup className="h-full min-h-0" orientation="horizontal">
            ...
            >
              <ReviewGroupRail
                ...
                query={query}
                viewedGroups={resolvedViewedGroups}
              />
            </ResizablePanel>
            <ResizableHandle />
            <ResizablePanel
              className="min-w-0 overflow-hidden"
              defaultSize={hasSupportingEvidenceContent ? "53.9%" : "79.3%"}
              id="review-document"
              minSize="560px"
            >
              {guideDocument}
      ...
      <Sheet onOpenChange={setGroupSheetOpen} open={groupSheetOpen}>
        <SheetContent
          className="data-[side=left]:w-[min(92vw,22rem)] p-0"
          showCloseButton={false}
          side="left"
        >
          <SheetHeader className="sr-only">
            <SheetTitle>Review groups</SheetTitle>
            <SheetDescription>Select a logical change group.</SheetDescription>
          </SheetHeader>
          <ReviewGroupRail
            activeGroupId={activeGroup.id}
            diffByPath={diffByPath}
