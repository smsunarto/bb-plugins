...
export function GuideContent({ blocks }: { blocks: readonly WalkthroughGuideBlock[] }) {
  ...
  for (const block of blocks) {
    ...
    if (block.type === "list") {
      ...
      content.push(
        <List
          className={`space-y-1 pl-5 text-sm leading-6 text-muted-foreground ${block.ordered ? "list-decimal" : "list-disc"}`}
          key={key}
        >
      ...
      content.push(
        <pre
          className="overflow-x-auto rounded-md border bg-muted/40 p-3 font-mono text-xs leading-5"
          key={key}
        >
