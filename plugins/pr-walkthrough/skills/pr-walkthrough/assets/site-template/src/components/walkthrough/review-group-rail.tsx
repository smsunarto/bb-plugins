...
export function ReviewGroupRail({
  activeGroupId,
  diffByPath,
  groups,
  onClose,
    ...
    <nav className="h-full min-h-0 bg-card" aria-label="Changes in pull request">
      <div className={`flex h-12 items-center justify-between gap-3 ${onClose ? "pl-4" : "px-4"}`}>
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">Changes in PR</h2>
        </div>
        {onClose ? (
          <Button
            aria-label="Close review groups"
                ...
                >
                  ...
                  <ItemContent className="min-w-0 gap-0">
                    <div className="flex items-start justify-between gap-2">
                      <ItemTitle className="line-clamp-none text-[13px] leading-5">
                        {group.title}
                      </ItemTitle>
                      {viewed ? (
                        <ItemActions className="mt-0.5 shrink-0">
                          <Badge
                            className="rounded-md border-[var(--added)]/50 text-[var(--added)]"
                            variant="outline"
                          >
                            <Check />
                            Reviewed
                    ...
                    <ItemDescription
                      className={`mt-2 leading-[18px] ${active ? "line-clamp-3" : "line-clamp-2"}`}
                    >
                      {group.summary}
                    </ItemDescription>
                  </ItemContent>
                ...
                {active && files.length > 0 ? (
                  <ItemGroup className="gap-0 border-t bg-background/35 px-2 py-1">
                    {orderedFiles.slice(0, FILE_PREVIEW_LIMIT).map((file) => (
                      <Item
                        className="min-h-0 gap-1.5 border-0 px-1.5 py-1"
                        key={file.path}
                        size="xs"
                      >
                        <ItemMedia className="size-4" variant="icon">
                          <FileCode2 className="size-3 text-muted-foreground" />
                        </ItemMedia>
                        <ItemContent className="min-w-0">
                          <ItemTitle className="block truncate font-mono text-[11px] font-normal leading-4">
                    ...
                    {orderedFiles.length > FILE_PREVIEW_LIMIT ? (
                      <Item
                        className="min-h-0 border-0 px-1.5 py-1 font-mono text-[10px] font-normal text-muted-foreground"
                        size="xs"
                      >
