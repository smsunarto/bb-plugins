...
function getPhaseProgress(phase: WalkthroughGuidePhase, reviewedExcerptIds: Set<string>) {
  ...
  return {
    optionalLabel:
      optional.length > 0 ? `${optionalViewed} of ${optional.length} optional viewed` : undefined,
    requiredLabel:
      required.length > 0 ? `${requiredReviewed} of ${required.length} hunks reviewed` : undefined,
  };
}
    ...
    <article className="space-y-3" data-guide-excerpt-id={excerpt.id}>
      ...
      <ReviewDiffSurface<WalkthroughGuideComment>
        ...
        renderAnnotation={(annotation) =>
          annotation.metadata ? <GuideLineComment comment={annotation.metadata} /> : null
        }
        reviewed={reviewed}
...
function GuidePhaseSection(
  props: Omit<ComponentProps<typeof PhaseContents>, "phase"> & { phase: WalkthroughGuidePhase },
) {
  const { phase } = props;
  ...
  return (
    <section data-guide-phase-id={phase.id}>
      <Accordion
        collapsible
        onValueChange={(value) => setOpen(value === phase.id)}
        type="single"
        value={open ? phase.id : ""}
      >
...
export function GuideDocument({
  ...
  const diffByPath = useMemo(
    () => new Map(groupFiles.map((file) => [file.path, file])),
    [groupFiles],
  );
        ...
        <div className="min-w-0">
          ...
          <ol className="mt-2 grid gap-x-6 gap-y-1 sm:grid-cols-2">
            {phases.map((phase, index) => {
              ...
              const countLabel = [progress.requiredLabel, progress.optionalLabel]
                .filter(Boolean)
                .join(" · ");
              return (
                <li
                  className="flex min-w-0 items-baseline justify-between gap-3 py-1 text-xs"
                  key={phase.id}
                >
