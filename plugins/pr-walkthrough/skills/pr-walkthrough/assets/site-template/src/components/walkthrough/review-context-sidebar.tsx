...
export function hasSupportingEvidence(
  ...
  return (
    groupFiles.some((file) => !file.generated && !file.binary && isSpecPath(file.path)) ||
...
export function ReviewContextSidebar({
    ...
    <div className={inline ? "space-y-4" : "space-y-4 p-4"}>
      {specs.length + group.links.length > 0 ? (
        <section className="space-y-2" aria-labelledby="related-evidence-heading">
          <h3 className="text-xs font-medium" id="related-evidence-heading">
            Relevant specs and links
          </h3>
          {specs.length > 0 ? (
            <EvidenceRows files={specs} icon={FileText} onOpenDiff={onOpenDiff} />
          ) : null}
      ...
      {group.comments.length > 0 ? (
        <section className="space-y-2" aria-labelledby="review-notes-heading">
          <h3 className="text-xs font-medium" id="review-notes-heading">
            Existing review notes
          </h3>
          <ItemGroup className="gap-2">
            {group.comments.map((comment) => (
              <Item
                className={cn(REVIEW_SURFACE_CLASS, "items-start bg-muted/50")}
  ...
  if (inline) {
    return (
      <aside aria-label="Supporting evidence" data-inline-evidence>
        {evidenceContent}
      </aside>
    );
  }
