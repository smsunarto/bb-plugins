...
export function ReviewDiffSurface<LAnnotation = undefined>({
  ...
  const options = createDiffOptions<LAnnotation>(
    diffStyle,
    !expanded,
    useFullFileContext ? file : undefined,
  );
  const annotationCount = lineAnnotations?.length ?? 0;

  function handleDiffClick(event: ReactMouseEvent<HTMLDivElement>) {
    const clickedHeader = event.nativeEvent
      .composedPath()
      .some((target) => target instanceof HTMLElement && target.hasAttribute("data-diffs-header"));
  ...
  return (
    ...
    >
      <div
        className="diff-viewport w-full min-w-0 max-w-full overflow-hidden"
        ...
        <div className="inline-diff w-full min-w-0 max-w-full" onClick={handleDiffClick}>
          <PatchDiff<LAnnotation>
            ...
            renderHeaderFilenameSuffix={() =>
              note || rangeLabel || (!expanded && annotationCount > 0) ? (
                <span className="inline-flex min-w-0 items-center gap-2 truncate text-xs text-muted-foreground">
                  {rangeLabel ? (
                    <span className="hidden shrink-0 font-mono sm:inline">{rangeLabel}</span>
                  ) : null}
                  {note ? (
                    <span className="hidden max-w-[28rem] truncate xl:inline" title={note}>
                      {note}
                    </span>
                  ) : null}
                  {!expanded && annotationCount > 0 ? (
                    <span
                      ...
                      title={`${annotationCount} read-only ${annotationCount === 1 ? "note" : "notes"}`}
                    >
            ...
            renderHeaderMetadata={() => (
              <Toggle
                aria-label={
                  reviewed ? `Mark ${fileName} as not reviewed` : `Mark ${fileName} reviewed`
                }
              ...
              >
                {reviewed ? (
                  ...
                  <Square className="size-4 text-foreground/50" />
                )}
                <span>Viewed</span>
              </Toggle>
            )}
            renderHeaderPrefix={() => (
              ...
              >
                <ChevronDown
                  className={`h-4 w-2.5 transition-transform ${expanded ? "" : "-rotate-90"}`}
                />
              </button>
            )}
          />
      ...
      {expanded && file.binary ? (
        <div className="border-t p-3">
          <Alert>
            <FileCode2 />
            <AlertDescription>
              Binary change. Open the pull request to inspect its preview.
            </AlertDescription>
          </Alert>
        </div>
      ) : null}
...
export function SourceDiff({
  ...
  const noteByPath = useMemo(
    () => new Map(evidence.map((file) => [file.path, file.note])),
    [evidence],
  );
  ...
  return <div className="space-y-3">{files.map(renderFile)}</div>;
}
...
export function GeneratedSourceDiff(props: SourceDiffProps) {
  ...
  return (
    <section className="pb-6 pt-3" data-generated-section>
      <Accordion
        collapsible
        onValueChange={(value) => setOpen(value === "generated-files")}
        type="single"
        value={open ? "generated-files" : ""}
      >
