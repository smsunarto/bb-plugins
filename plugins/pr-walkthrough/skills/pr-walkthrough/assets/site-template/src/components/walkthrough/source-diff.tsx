...
export function ReviewDiffSurface<LAnnotation = undefined>({
  ...
  return (
    <section
      ...
      data-generated={file.generated ? "true" : "false"}
...
export function SourceDiff({
  ...
  function toggleExpanded(path: string) {
    const next = new Set(filePaths.filter((filePath) => expandedPaths.has(filePath)))
    if (next.has(path)) next.delete(path)
    else next.add(path)
    onExpandedPathsChange(filePaths.filter((filePath) => next.has(filePath)))
  }
