...
- Tailwind CSS 4, Oxlint, TypeScript, Bun, and exact dependency versions.
...
Use a neutral, product-independent interface. Do not copy vendor names, logos, competitor assets, review findings, or merge controls. Use the monospaced stack only for code and monospaced metadata. The template bundles no font binary: it asks for a locally installed Berkeley Mono through `local()` sources and falls back to the system monospace stack. Do not hotlink, bundle, or redistribute font files.
...
```bash
bun run <skill-directory>/scripts/serve-static.ts \
  --directory .pr-walkthrough/site/out \
  --bind 127.0.0.1 \
  --port 4173
```
