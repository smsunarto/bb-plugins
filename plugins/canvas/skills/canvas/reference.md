...
| `DiffView`  | Unified patch from a fenced diff block, rendered by BB's SDK diff viewer in the active code theme with a collapsible header. | one fenced code block, stored as `patch`   | no        |
