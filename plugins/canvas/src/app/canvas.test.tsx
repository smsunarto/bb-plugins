...
test("pills in a row sit one comment anchor below the row, where the CSS keeps them narrow", async () => {
  const slot = renderSlot({ component: CanvasOpener }, propsFor("canvases/labels.canvas.mdx"), {
    rpc: {
      render: () =>
        rendered(
  ...
  await slot.findByText("size/M");
  assert.equal(
    slot.container.querySelectorAll(".canvas-row > .canvas-comment-block > .canvas-pill").length,
    2,
  );
  slot.unmount();
});
