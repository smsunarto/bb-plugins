...
export async function createRemoteSessionResponse(
  ...
  if (
    ...
    // A response from localhost cannot install a cookie for getbb.app. A
    // Connect-authenticated request can because the edge forwards the response
    // to its original getbb.app URL after rewriting the origin-facing host.
