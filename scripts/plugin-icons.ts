...
const paletteIcon: readonly IconNode[] = [
  ["path", {
    ...
    fill: "none", fillRule: "evenodd", clipRule: "evenodd", stroke: "currentColor",
  ...
  ["path", {
    ...
    stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5", key: "1",
  }],
  ["path", {
    ...
    stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5", key: "2",
  }],
  ["path", {
    ...
    stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5", key: "3",
  }],
  ["path", {
    ...
    stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5", key: "4",
  }],
  ["path", {
    ...
    stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5", key: "5",
  }],
  ["path", {
    ...
    stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5", key: "6",
  }],
];
...
async function expectedFiles(plugin: CustomPlugin): Promise<Map<string, string>> {
  const { nodes } = customIcons[plugin];
  const directory = resolve("plugins", plugin, "assets");
  return new Map([
    [resolve(directory, "icon.svg"), svg(nodes, "#000", false)],
    [resolve(directory, "logo.svg"), svg(nodes, themeColors.light, true)],
    [resolve(directory, "logo-dark.svg"), svg(nodes, themeColors.dark, true)],
  ]);
}

async function validateManifest(plugin: string, expectedIcon: string): Promise<string[]> {
  const path = resolve("plugins", plugin, "package.json");
  ...
  if (expectedIcon.startsWith(".") &&
      (branding?.logo?.light !== "./assets/logo.svg" || branding?.logo?.dark !== "./assets/logo-dark.svg")) {
...
for (const plugin of Object.keys(customIcons) as CustomPlugin[]) {
  ...
  console.log(`${plugin.padEnd(16)} Hugeicons ${customIcons[plugin].name}`);
}
